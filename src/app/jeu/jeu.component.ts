...
export class JeuComponent implements OnInit {
    ...
    clickSound = new Audio(
        '.\\assets\\sounds\\Button_Press_4-Marianne_Gagnon-570460555.mp3'
    );
    ...
    touchLeft(): void {
        if (
            this.seaWorm.getDirection() != Direction.droite &&
            !this.controlPressed
        ) {
    ...
    touchRight(): void {
        if (
            this.seaWorm.getDirection() !== Direction.gauche &&
            !this.controlPressed
        ) {
    ...
    touchUp(): void {
        if (
            this.seaWorm.getDirection() !== Direction.bas &&
            !this.controlPressed
        ) {
    ...
    touchDown(): void {
        if (
            this.seaWorm.getDirection() !== Direction.haut &&
            !this.controlPressed
        ) {
    ...
    runGameCycle(): void {
        ...
        if (this.isBitten === false) {
            ...
            if (
                this.food.getCase().getId() ===
                this.seaWorm.getCases()[0].getId()
            ) {
                this.playEatSound();
                this.vibrateDevice();
    ...
    getFoodEffect(foodtype: TYPE): void {
        switch (foodtype) {
            ...
            case TYPE.green:
                if (
                    this.gameService.getLevel() === 225 ||
                    this.gameService.getLevel() === 300
                ) {
    ...
    checkBites(): boolean {
        if (this.seaWorm.getCases().length >= 2) {
            ...
            if (
                (head.getId() == this.poison1.getCase().getId() &&
                    this.poison1.getToShow()) ||
                (head.getId() == this.poison2.getCase().getId() &&
                    this.poison2.getToShow()) ||
                (head.getId() == this.poison3.getCase().getId() &&
                    this.poison3.getToShow())
            ) {
                return true;
            }
        }
    ...
    mediumShrinkWorm(): void {
        if (this.seaWorm.getCases().length > 6) {
            const wormShrinked = this.seaWorm
                .getCases()
                .slice(0, this.seaWorm.getCases().length - 3);
            this.seaWorm.setCases(wormShrinked);
        }
        this.food.setType(TYPE.yellowgreen);
    }
    ...
    shrinkWorm(): void {
        if (this.seaWorm.getCases().length > 4) {
            const wormShrinked = this.seaWorm
                .getCases()
                .slice(0, this.seaWorm.getCases().length - 2);
            this.seaWorm.setCases(wormShrinked);
        }
        this.food.setType(TYPE.yellowgreen);
    }
    ...
    growWorm(): void {
        ...
        let pixelToAdd;
        const lastPixelWorm = this.seaWorm.getCases()[
            this.seaWorm.getCases().length - 1
        ];
        if (this.seaWorm.getCases().length > 1) {
            const beforeLastPixelWorm = this.seaWorm.getCases()[
                this.seaWorm.getCases().length - 2
            ];
            pixelToAdd = new Case(
                State.worm,
                lastPixelWorm.getPositionX() -
                    (beforeLastPixelWorm.getPositionX() -
                        lastPixelWorm.getPositionX()),
                lastPixelWorm.getPositionY() -
                    (beforeLastPixelWorm.getPositionY() -
                        lastPixelWorm.getPositionY())
            );
    ...
    extraGrowWorm(): void {
        ...
        const pixelToAdd = new Case(
            State.worm,
            lastPixel.getPositionX() -
                (beforeLastPixelWorm.getPositionX() - lastPixel.getPositionX()),
            lastPixel.getPositionY() -
                (beforeLastPixelWorm.getPositionY() - lastPixel.getPositionY())
        );
        bigWorm.push(pixelToAdd);
        this.seaWorm.setCases(bigWorm);
    }
        ...
        for (const pixel of this.grid) {
            ...
            if (
                (this.poison1 != undefined &&
                    ...
                    this.poison1.getToShow()) ||
                (pixel.getId() == this.poison2.getCase().getId() &&
                    this.poison2.getToShow()) ||
                (pixel.getId() == this.poison3.getCase().getId() &&
                    this.poison3.getToShow())
            ) {
            ...
            if (i == 0) {
                ...
                switch (this.seaWorm.getDirection()) {
                    case Direction.gauche:
                        ...
                        pixelToMove = new Case(
                            State.worm,
                            positionX,
                            newWormCase.getPositionY()
                        );
                        break;
                    case Direction.droite:
                        positionX = newWormCase.getPositionX() + 1;
