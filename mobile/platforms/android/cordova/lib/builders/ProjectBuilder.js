...
class ProjectBuilder {
    ...
    extractRealProjectNameFromManifest() {
        ...
        if (!m) {
            throw new CordovaError(
                'Could not find package name in ' + manifestPath
            )
        }
    ...
    prepBuildFiles() {
        ...
        subProjects.forEach(function (p) {
            ...
            if (libName !== 'app') {
                depsList +=
                    '    implementation(project(path: ":' + libName + '"))'
                insertExclude(p)
            }
        })
        ...
        propertiesObj.systemLibs.forEach(function (p) {
            ...
            if (/:.*:/.exec(p)) {
                ...
                if (!mavenRef) {
                    throw new CordovaError(
                        'Unsupported system library (does not work with gradle): ' +
                            p
                    )
                }
            }
        ...
        return spawn(wrapper, args, { stdio: 'pipe' })
            ...
            .catch(function (error) {
                if (
                    error
                        .toString()
                        .indexOf('failed to find target with hash string') >= 0
                ) {
                    return check_reqs
                        .check_android_target(error)
                        .then(function () {
    ...
    findOutputApks(build_type, arch) {
        return findOutputApksHelper(this.apkDir, build_type, arch).sort(
            apkSorter
        )
    }
    ...
    var ret = shell
        ...
        .filter(function (candidate) {
            ...
            if (build_type === 'debug') {
                return (
                    /-debug/.exec(apkName) &&
                    !/-unaligned|-unsigned/.exec(apkName)
                )
            }
