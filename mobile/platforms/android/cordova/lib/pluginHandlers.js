    ...
    framework: {
        ...
        uninstall: function (obj, plugin, project, options) {
            ...
            if (obj.custom) {
                ...
                if (
                    fs.existsSync(parDir) &&
                    fs.readdirSync(parDir).length === 0
                ) {
                    fs.rmdirSync(parDir)
                }
            } else {
    ...
    asset: {
        ...
        uninstall: function (obj, plugin, project, options) {
            ...
            if (options && options.usePlatformWww) {
                ...
                removeFileF(
                    path.resolve(project.platformWww, 'plugins', plugin.id)
                )
            }
        },
    },
...
module.exports.getInstaller = function (type) {
    ...
    events.emit(
        'verbose',
        '<' + type + '> is not supported for android plugins'
    )
}
