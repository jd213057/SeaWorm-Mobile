...
module.exports.prepare = function (cordovaProject, options) {
    ...
    const minSdkVersion = this._config.getPreference(
        'android-minSdkVersion',
        'android'
    )
    const maxSdkVersion = this._config.getPreference(
        'android-maxSdkVersion',
        'android'
    )
    const targetSdkVersion = this._config.getPreference(
    ...
    if (minSdkVersion)
        gradlePropertiesUserConfig.cdvMinSdkVersion = minSdkVersion
    if (maxSdkVersion)
        gradlePropertiesUserConfig.cdvMaxSdkVersion = maxSdkVersion
    if (targetSdkVersion)
        gradlePropertiesUserConfig.cdvTargetSdkVersion = targetSdkVersion
...
function updateProjectAccordingTo(platformConfig, locations) {
    ...
    events.emit(
        'verbose',
        'Wrote out android application name "' +
            name +
            '" to ' +
            locations.strings
    )
...
function getAdaptiveImageResourcePath(
    resourcesDir,
    type,
    density,
    name,
...
function cleanSplashes(projectRoot, projectConfig, platformResourcesDir) {
    ...
    if (resources.length > 0) {
        ...
        events.emit(
            'verbose',
            'Cleaning splash screens at ' + platformResourcesDir
        )
...
function updateIcons(cordovaProject, platformResourcesDir) {
    ...
    resourceMap = updateIconResourceForLegacy(
        preparedIcons,
        resourceMap,
        platformResourcesDir
    )
...
function updateIconResourceForLegacy(
    preparedIcons,
    resourceMap,
    platformResourcesDir
) {
