            ...
            for (var i = 1; i < response.length; i++) {
                ...
                if (response[i].match(/Name:\s/)) {
                    ...
                    if (response[i + 1].match(/Device:\s/)) {
                        i++
                        img_obj['device'] = response[i]
                            .split('Device: ')[1]
                            .replace('\r', '')
                    }
                    if (response[i + 1].match(/Path:\s/)) {
                        i++
                        img_obj['path'] = response[i]
                            .split('Path: ')[1]
                            .replace('\r', '')
                    }
                    if (response[i + 1].match(/Target:\s/)) {
                        ...
                        if (response[i + 1].match(/Based\son:\s/)) {
                            img_obj['target'] = response[i + 1].split(
                                'Based on:'
                            )[1]
                            if (img_obj['target'].match(/Tag\/ABI:\s/)) {
                                ...
                                if (img_obj['target'].indexOf('(') > -1) {
                                    img_obj['target'] = img_obj['target']
                                        .substr(
                                            0,
                                            img_obj['target'].indexOf('(') - 1
                                        )
                                        .trim()
                                }
                            }
                            var version_string = img_obj['target'].replace(
                                /Android\s+/,
                                ''
                            )

                            var api_level =
                                android_sdk.version_string_to_api_level[
                                    version_string
                                ]
                            if (api_level) {
                                img_obj['target'] +=
    ...
    return this.list_images().then(function (images) {
        ...
        for (var i in images) {
            ...
            if (target && target.indexOf('API level') > -1) {
                var num = parseInt(
                    target.split('(API level ')[1].replace(')', '')
                )
                if (num === project_target) {
                    return images[i]
                } else if (
                    project_target - num < closest &&
                    project_target > num
                ) {
...
module.exports.start = function (emulator_ID, boot_timeout) {
    ...
    return Promise.resolve()
        ...
        .then(function (emulatorId) {
            if (!emulatorId) {
                return Promise.reject(
                    new CordovaError('Failed to start emulator')
                )
            }
            ...
            return self
                ...
                .then(function (success) {
                    if (success) {
                        ...
                        // unlock screen
                        return Adb.shell(emulatorId, 'input keyevent 82').then(
                            function () {
            ...
            return Promise.resolve().then(function () {
                ...
                function adbInstallWithOptions(target, apk, opts) {
                    ...
                    return new Promise(function (resolve, reject) {
                        child_process.exec(command, opts, function (
                            err,
                            stdout,
                            stderr
                        ) {
                            ...
                            // message to stdout, so we have to use RegExp matching to detect installation failure.
                                ...
                                reject(
                                    new CordovaError(
                                        'Failed to install apk to emulator: ' +
                                            stdout
                                    )
                                )
                            } else resolve(stdout)
                ...
                function installPromise() {
                    return adbInstallWithOptions(
                        ...
                        // This promise is always resolved, even if 'adb uninstall' fails to uninstall app
                        // or the app doesn't installed at all, so no error catching needed.
                        return Adb.uninstall(target.target, pkgName).then(
                            function () {
                                return adbInstallWithOptions(
                                    target.target,
                                    apk_path,
