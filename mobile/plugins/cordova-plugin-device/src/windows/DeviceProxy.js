...
var ROOT_CONTAINER_QUERY =
    'System.Devices.ContainerId:="' + ROOT_CONTAINER + '"'
...
module.exports = {
    getDeviceInfo: function (win, fail, args) {
        ...
        if (localSettings.values.deviceId) {
            ...
            try {
                var ASHWID = Windows.System.Profile.HardwareIdentification.getPackageSpecificToken(
                    null
                ).id
                deviceId = Windows.Storage.Streams.DataReader.fromBuffer(
                    ASHWID
                ).readGuid()
            } catch (e) {
        ...
        var devicePlatform =
            userAgent.indexOf('MSAppHost/1.0') === -1 ? 'windows' : 'windows8'
        var versionString = userAgent.match(
        ...
        Pnp.PnpObject.findAllAsync(
            ...
            for (var i = 0; i < rootDevices.length; i++) {
                ...
                if (
                    rootDevice.properties[DEVICE_CLASS_KEY_NO_SEMICOLON] ===
                    HAL_DEVICE_CLASS
                ) {
                    versionString =
